...
4. Запусти НЕЗАВИСИМОГО тестировщика отдельным процессом:

   if [ -f ~/.claude/.tg-anthropic-key ]; then set -a; . ~/.claude/.tg-anthropic-key; set +a; fi; \\
   ...
   КРИТИЧНО по запуску:
   - Запускай тестер ТОЛЬКО в ФОРГРАУНДЕ (блокирующе) и дождись его завершения. \
НЕ используй run_in_background / фоновый запуск (`&`, nohup, отдельный воркер) — \
между ходами фоновый дочерний процесс прибивается враппером, тест умирает без кейсов.
   - Вызови его через Bash с таймаутом ~600000 мс (10 минут), а не дефолтным — \
браузерный прогон длится минуты, дефолтный таймаут его обрежет.
   - Авторизация: тестер берёт креды Claude этой машины (как обычный claude auth); \
строка с `. ~/.claude/.tg-anthropic-key` — подстраховка, сохрани её.
